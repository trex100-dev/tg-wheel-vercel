...
async function addGuaranteesIfNeeded(tx, userId) {
  ...
  const u = await tx(`SELECT total_spent FROM users WHERE user_id=$1 FOR UPDATE`, [String(userId)]);
  const p = await tx(`SELECT * FROM user_progress WHERE user_id=$1 FOR UPDATE`, [String(userId)]);
  ...
  await tx(
    `UPDATE user_progress
     SET trig_prize_3=$1,
         ...
         trig_prize_4=$3,
         trig_prize_5=$4,
         guarantee_queue=$5::jsonb
     WHERE user_id=$6`,
    [prog.trig_prize_3, prog.trig_prize_6, prog.trig_prize_4, prog.trig_prize_5, JSON.stringify(queue), String(userId)]
  );
}
...
async function selectPrizeId(tx, userId) {
  ...
  const u = await tx(`SELECT total_spent FROM users WHERE user_id=$1 FOR UPDATE`, [String(userId)]);
  const p = await tx(`SELECT guarantee_queue FROM user_progress WHERE user_id=$1 FOR UPDATE`, [String(userId)]);
  ...
  if (queue.length > 0) {
    ...
    await tx(
      `UPDATE user_progress
       SET guarantee_queue=$1::jsonb
       WHERE user_id=$2`,
      [JSON.stringify(newQueue), String(userId)]
    );
    return g;
  }
